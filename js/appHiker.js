...
function pasarBinarioEntera(numeroDecimal) {
  // Convierte la parte entera del numero en binario.
  ...
  while (parteEntera > 0) {
    // Mientras la parte entera sea mayor que 0, se divide entre 2 y se recoge el resto, concatenando el resto anterior.
...
function pasarBinarioDecimal(numeroDecimal) {
  // Convierte la parte decimal del numero en binario.

  var numeroPositivo = numeroDecimal;

  if (numeroPositivo < 0) {
    // Si el numero es negativo, lo convierte a positivo para poder usarlo.
  ...
  while (contadorCantidadNumeros <= 150) {
    ...
    parteDecimalBinario =
      parteDecimalBinario + Math.floor(resultadoMultiplicacion);
    parteDecimal =
      (Math.floor(resultadoMultiplicacion) - resultadoMultiplicacion) * -1;
  }
...
function binarioCompleto(numeroDecimal) {
  numeroBinario =
    pasarBinarioEntera(numeroDecimal) +
    "." +
    pasarBinarioDecimal(numeroDecimal);

  return numeroBinario;
}

function pasarBinarioCientifica() {
  // Pasamos el binario obtenido a notación cientifica.
  var posicionUno;
  var inicio;
  var final;
  if (numeroBinario.indexOf("1") !== -1) {
    // Guardamos la posicion del primer "1" del binario.
    posicionUno = numeroBinario.indexOf("1");
  } else {
    posicionUno = 127; //En caso de que no haya ningún 1, el valor por defecto será 127, para que el exponente final sea 1
  }
  ...
  if (numeroBinario.substring(0, posicionPunto) === "0") {
    inicio = numeroBinario.substring(posicionUno, posicionUno + 1);
    final =
      "." + numeroBinario.substring(posicionUno + 1, numeroBinario.length);
    ...
    inicio = numeroBinario.substring(0, posicionUno);
    final = numeroBinario.substring(posicionUno, numeroBinario.length);
...
function ieee754aBinario(numeroDecimal) {
  // Por último teniendo todos los datos necesarios, pasamos el numero al estandar IEEE754
  ...
  if (numeroDecimal >= 0) {
    // Para determinar el signo, tan simple como comprobar si el numero inicial es positivo o negativo.
    signo = "0";
  } else {
    signo = "1";
  }
  ...
  while (mantisa.length < 23) {
    // En caso de que la mantisa no llegue a 23 números, rellenaremos con "0" a la derecha hasta completarla.
    mantisa = mantisa + "0";
  }
